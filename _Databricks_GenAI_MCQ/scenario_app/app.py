...
def render_scenario(index: int):
    ...
    # Button to reveal the official answer for this scenario
    answer_flag = f"show_answer_{index}"
    if answer_flag not in st.session_state:
        st.session_state[answer_flag] = False
    if not st.session_state[answer_flag]:
        if st.button("Show Answer", key=f"show_ans_{index}"):
            st.session_state[answer_flag] = True
            # Rerun to display the answer immediately
            try:
                st.rerun()
            except Exception:
                pass
    else:
        # Display the official answer and reference
...
def main():
    # Configure the page only once; catch repeated calls when embedded in a wrapper app
    try:
    ...
    # Load scenarios
    # Load the updated scenarios dataset (including additional questions)
    scenario_path = os.path.join(os.path.dirname(__file__), "scenarios_updated.json")
